...
def normal_distribution():
    ...
    """
    # Läser in csv-filerna till pandas dataframes
    ...
    # Skapar ett histogram av datan i varje kolumn
    ...
    # Beräknar medelvärdet och standardavvikelsen för varje kolumn
    mean1, std1 = norm.fit(values1)
    mean2, std2 = norm.fit(values2)
    mean3, std3 = norm.fit(values3)

    # Skapar en normalfördelning för varje kolumn baserat på medelvärdet och standardavvikelsen
    ...
    # Ritar den normalfördelade kurvan för varje kolumn
    ...
    # Lägger till etiketter och legend
...
def box_plot():
    ...
    # Skapar en lådaplot med data från varje kolumn
    plt.boxplot(
        [values1, values2, values3], labels=["Gladhammar", "Målilla", "Gotska Sandö"]
    )

    # Lägger till etiketter
    plt.xlabel("Values")
    plt.ylabel("Value")
